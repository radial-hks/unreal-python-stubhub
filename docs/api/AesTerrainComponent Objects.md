## AesTerrainComponent Objects

```python
class AesTerrainComponent(PrimitiveComponent)
```
...
- ``on_terrain_data_layer_changed`` (OnTerrainDataLayerChanged):  [Read-Write]
...
- ``terrain_material`` (MaterialInterface):  [Read-Write] 地形材质
- ``terrain_material_ranges`` (Array[AesMaterialLevelRange]):  [Read-Write] 地形材质范围配置
  注：当多个 Range 覆盖同一 Level 时，数组中后面的 Range 会覆盖前面的 Range（后覆盖优先）
- ``tile_grid_hem_num`` (int32):  [Read-Write]
...
- ``under_water_material`` (MaterialInterface):  [Read-Write] 水下材质
...
- ``water_highlight_rt`` (TextureRenderTarget2D):  [Read-Only]
- ``water_material`` (MaterialInterface):  [Read-Write] 水面材质
- ``water_material_ranges`` (Array[AesMaterialLevelRange]):  [Read-Write] 水面材质范围配置
  注：当多个 Range 覆盖同一 Level 时，数组中后面的 Range 会覆盖前面的 Range（后覆盖优先）
- ``water_outline_rt`` (TextureRenderTarget2D):  [Read-Only]
- ``water_visible_rt`` (TextureRenderTarget2D):  [Read-Only]

<a id="unreal.AesTerrainComponent.terrain_material"></a>

#### terrain\_material

```python
@property
def terrain_material() -> MaterialInterface
```

(MaterialInterface):  [Read-Write] 地形材质
...
```python
@terrain_material.setter
def terrain_material(value: MaterialInterface) -> None
```

<a id="unreal.AesTerrainComponent.terrain_material_ranges"></a>

#### terrain\_material\_ranges

```python
@property
def terrain_material_ranges() -> Array[AesMaterialLevelRange]
```

(Array[AesMaterialLevelRange]):  [Read-Write] 地形材质范围配置
注：当多个 Range 覆盖同一 Level 时，数组中后面的 Range 会覆盖前面的 Range（后覆盖优先）
...
```python
@terrain_material_ranges.setter
def terrain_material_ranges(value: Array[AesMaterialLevelRange]) -> None
```

<a id="unreal.AesTerrainComponent.water_material"></a>
...
(MaterialInterface):  [Read-Write] 水面材质
...
<a id="unreal.AesTerrainComponent.water_material_ranges"></a>

#### water\_material\_ranges

```python
@property
def water_material_ranges() -> Array[AesMaterialLevelRange]
```

(Array[AesMaterialLevelRange]):  [Read-Write] 水面材质范围配置
注：当多个 Range 覆盖同一 Level 时，数组中后面的 Range 会覆盖前面的 Range（后覆盖优先）
...
```python
@water_material_ranges.setter
def water_material_ranges(value: Array[AesMaterialLevelRange]) -> None
```

<a id="unreal.AesTerrainComponent.under_water_material"></a>

#### under\_water\_material

```python
@property
def under_water_material() -> MaterialInterface
```

(MaterialInterface):  [Read-Write] 水下材质
...
```python
@under_water_material.setter
def under_water_material(value: MaterialInterface) -> None
```

<a id="unreal.AesTerrainComponent.min_height"></a>
...
<a id="unreal.AesTerrainComponent.max_height"></a>
...
<a id="unreal.AesTerrainComponent.tile_grid_hem_num"></a>

#### tile\_grid\_hem\_num

```python
@property
def tile_grid_hem_num() -> int
```
...
```python
@tile_grid_hem_num.setter
def tile_grid_hem_num(value: int) -> None
```

<a id="unreal.AesTerrainComponent.lod_view_actors"></a>

#### lod\_view\_actors

```python
@property
def lod_view_actors() -> Array[Actor]
```
...
```python
@lod_view_actors.setter
def lod_view_actors(value: Array[Actor]) -> None
```

<a id="unreal.AesTerrainComponent.water_visible_rt"></a>

#### water\_visible\_rt

```python
@property
def water_visible_rt() -> TextureRenderTarget2D
```

(TextureRenderTarget2D):  [Read-Only]

<a id="unreal.AesTerrainComponent.water_highlight_rt"></a>

#### water\_highlight\_rt

```python
@property
def water_highlight_rt() -> TextureRenderTarget2D
```

(TextureRenderTarget2D):  [Read-Only]

<a id="unreal.AesTerrainComponent.water_outline_rt"></a>

#### water\_outline\_rt

```python
@property
def water_outline_rt() -> TextureRenderTarget2D
```

(TextureRenderTarget2D):  [Read-Only]

<a id="unreal.AesTerrainComponent.on_terrain_data_layer_changed"></a>

#### on\_terrain\_data\_layer\_changed

```python
@property
def on_terrain_data_layer_changed() -> OnTerrainDataLayerChanged
```

(OnTerrainDataLayerChanged):  [Read-Write]
...
```python
@on_terrain_data_layer_changed.setter
def on_terrain_data_layer_changed(value: OnTerrainDataLayerChanged) -> None
```

<a id="unreal.AesTerrainComponent.unregister_layer"></a>

#### unregister\_layer

```python
def unregister_layer(type: LayerType) -> None
```

x.unregister_layer(type) -> None
Unregister Layer

Args:
    type (LayerType):

<a id="unreal.AesTerrainComponent.set_level_water_material"></a>

#### set\_level\_water\_material

```python
def set_level_water_material(level: int, material: MaterialInterface) -> None
```

x.set_level_water_material(level, material) -> None
设置指定 Level 范围的水面材质

Args:
    level (int32): 指定的 Level
    material (MaterialInterface): 要设置的材质

<a id="unreal.AesTerrainComponent.set_level_terrain_material"></a>

#### set\_level\_terrain\_material

```python
def set_level_terrain_material(level: int,
                               material: MaterialInterface) -> None
```

x.set_level_terrain_material(level, material) -> None
设置指定 Level 范围的地形材质
...
<a id="unreal.AesTerrainComponent.remove_level_water_material"></a>

#### remove\_level\_water\_material

```python
def remove_level_water_material(level: int) -> None
```

x.remove_level_water_material(level) -> None
移除指定 Level 的水面材质配置

Args:
    level (int32): 指定的 Level

<a id="unreal.AesTerrainComponent.remove_level_terrain_material"></a>

#### remove\_level\_terrain\_material

```python
def remove_level_terrain_material(level: int) -> None
```

x.remove_level_terrain_material(level) -> None
移除指定 Level 的地形材质配置

Args:
    level (int32): 指定的 Level

<a id="unreal.AesTerrainComponent.register_layer"></a>

#### register\_layer

```python
def register_layer(type: LayerType, lod_type: LayerLODType) -> None
```

x.register_layer(type, lod_type) -> None
Register Layer

Args:
    type (LayerType): 
    lod_type (LayerLODType):

<a id="unreal.AesTerrainComponent.get_water_material_for_level"></a>

#### get\_water\_material\_for\_level

```python
def get_water_material_for_level(level: int) -> MaterialInterface
```

x.get_water_material_for_level(level) -> MaterialInterface
获取指定 Level 的水面材质
...
<a id="unreal.AesTerrainComponent.get_terrain_material_for_level"></a>

#### get\_terrain\_material\_for\_level

```python
def get_terrain_material_for_level(level: int) -> MaterialInterface
```

x.get_terrain_material_for_level(level) -> MaterialInterface
获取指定 Level 的地形材质
...
<a id="unreal.AesTerrainComponent.get_cache_size"></a>

#### get\_cache\_size

```python
def get_cache_size() -> Map[Name, int]
```

x.get_cache_size() -> Map[Name, int32]
Get Cache Size

Returns:
    Map[Name, int32]:

<a id="unreal.AesTerrainQuadMeshComponent"></a>