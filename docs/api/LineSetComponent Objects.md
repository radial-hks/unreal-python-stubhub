## LineSetComponent Objects

```python
class LineSetComponent(MeshComponent)
```
...
x.set_line_material(line_material) -> None
Specify material which handles lines

Args:
    line_material (MaterialInterface):

<a id="unreal.LineSetComponent.clear"></a>
...
def add_lines(start: Array[Vector],
              ...
              color: Color,
              thickness: float,
              depth_bias: float = 0.000000) -> int
```

x.add_lines(start, end, color, thickness, depth_bias=0.000000) -> int32
...
<a id="unreal.MeshWireframeComponent"></a>