## PCGPartitionActor Objects

```python
class PCGPartitionActor(PartitionActor)
```
...
- ``pcg_grid_size`` (uint32):  [Read-Only]
...
x.get_original_component(local_component) -> PCGComponent
Get Original Component

Args:
    local_component (PCGComponent): 
...
x.get_local_component(original_component) -> PCGComponent
...
Args:
    original_component (PCGComponent): 

Returns:
    PCGComponent:

<a id="unreal.PCGActorHelpers"></a>