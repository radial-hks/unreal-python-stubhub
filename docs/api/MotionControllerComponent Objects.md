## MotionControllerComponent Objects

```python
class MotionControllerComponent(PrimitiveComponent)
```
...
- **File**: MotionControllerComponent.h
...
- ``current_tracking_status`` (TrackingStatus):  [Read-Write] The tracking status for the device (e.g. full tracking, inertial tracking only, no tracking)
...
- ``disable_low_latency_update`` (bool):  [Read-Write] If false, render transforms within the motion controller hierarchy will be updated a second time immediately before rendering.
...
- ``motion_source`` (Name):  [Read-Write] Defines which pose this component should receive from the OpenXR Runtime. Left/Right MotionSource is the same as LeftGrip/RightGrip. See OpenXR specification for details on poses.
...
- ``player_index`` (int32):  [Read-Write] Which player index this motion controller should automatically follow
...
```python
@player_index.setter
def player_index(value: int) -> None
```
...
```python
@property
def motion_source() -> Name
```

(Name):  [Read-Write] Defines which pose this component should receive from the OpenXR Runtime. Left/Right MotionSource is the same as LeftGrip/RightGrip. See OpenXR specification for details on poses.
...
def motion_source(value: Name) -> None
```
...
```python
@property
def disable_low_latency_update() -> bool
```
...
def disable_low_latency_update(value: bool) -> None
```
...
```python
@property
def current_tracking_status() -> TrackingStatus
```
...
x.get_linear_velocity() -> Vector or None
...
x.get_linear_acceleration() -> Vector or None
...
Returns:
    ...
    out_linear_acceleration (Vector):
...
x.get_angular_velocity() -> Rotator or None
...
Returns:
    ...
    out_angular_velocity (Rotator):

<a id="unreal.Manipulator"></a>