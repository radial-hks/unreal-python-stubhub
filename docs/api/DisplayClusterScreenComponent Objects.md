## DisplayClusterScreenComponent Objects

```python
class DisplayClusterScreenComponent(StaticMeshComponent)
```
...
- **File**: DisplayClusterScreenComponent.h
...
- ``size`` (Vector2D):  [Read-Write] Adjust the size of the screen.
...
<a id="unreal.DisplayClusterScreenComponent.set_screen_size"></a>

#### set_screen_size

```python
def set_screen_size(size: Vector2D) -> None
```

x.set_screen_size(size) -> None
Set screen size (update transform scale).

Args:
    size (Vector2D):

<a id="unreal.DisplayClusterScreenComponent.get_screen_size"></a>

#### get_screen_size

```python
def get_screen_size() -> Vector2D
```

x.get_screen_size() -> Vector2D
Return the screen size adjusted by its transform scale.

Returns:
    Vector2D:

<a id="unreal.DisplayClusterStageGeometryComponent"></a>