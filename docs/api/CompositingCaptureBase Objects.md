## CompositingCaptureBase Objects

```python
class CompositingCaptureBase(CompositingElement)
```
...
- ``apply_distortion`` (bool):  [Read-Write] Whether to apply distortion as a post-process effect on this CG Layer
...
- ``lens_component_picker`` (ComponentReference):  [Read-Write] A component reference (customized) that allows the user to specify a component that this controller should control
...
- ``original_focal_length`` (float):  [Read-Write] Focal length of the target camera before any overscan has been applied
- ``outputs`` (Array[CompositingElementOutput]):  [Read-Write]
- ``overscan_factor`` (float):  [Read-Write] Value used to augment the FOV of the scene capture to produce a CG image with enough data to distort
...
- ``scene_capture_component2d`` (SceneCaptureComponent2D):  [Read-Only] Component used to generate the scene capture for this CG Layer
...
<a id="unreal.CompositingCaptureBase.scene_capture_component2d"></a>

#### scene_capture_component2d

```python
@property
def scene_capture_component2d() -> SceneCaptureComponent2D
```

(SceneCaptureComponent2D):  [Read-Only] Component used to generate the scene capture for this CG Layer

<a id="unreal.CompositingCaptureBase.apply_distortion"></a>
...
<a id="unreal.CompositingCaptureBase.overscan_factor"></a>
...
<a id="unreal.CompositingCaptureBase.original_focal_length"></a>

#### original_focal_length

```python
@property
def original_focal_length() -> float
```

(float):  [Read-Only] Focal length of the target camera before any overscan has been applied

<a id="unreal.CompositingCaptureBase.update_distortion"></a>

#### update_distortion

```python
def update_distortion() -> None
```
...
<a id="unreal.CompositingPickerAsyncTask"></a>