## CausticsGeneratorActor Objects

```python
class CausticsGeneratorActor(Actor)
```
...
- ``default_scene_root`` (SceneComponent):  [Read-Write] UPROPERTY(BlueprintReadWrite, NonTransactional, meta = (Category = "Default"))
...
```python
@property
def default_scene_root() -> SceneComponent
```

(SceneComponent):  [Read-Write] UPROPERTY(BlueprintReadWrite, NonTransactional, meta = (Category = "Default"))
UHierarchicalInstancedStaticMeshComponent* CausticParticlesGridHISMC;
...
def default_scene_root(value: SceneComponent) -> None
```
...
def spawn_water_preview_grid(hismc: HierarchicalInstancedStaticMeshComponent,
                             grid_size: float, grid_tiles: int) -> None
```

x.spawn_water_preview_grid(hismc, grid_size, grid_tiles) -> None
Spawn Water Preview Grid

Args:
    hismc (HierarchicalInstancedStaticMeshComponent): 
    grid_size (float): 
    grid_tiles (int32):
...
def spawn_caustic_particle_grid(
        hismc: HierarchicalInstancedStaticMeshComponent, grid_size: float,
        grid_tiles: int) -> None
```

x.spawn_caustic_particle_grid(hismc, grid_size, grid_tiles) -> None
...
x.editor_tick(delta_seconds) -> None
Editor Tick

Args:
    delta_seconds (float):

<a id="unreal.JumpFloodComponent2D"></a>