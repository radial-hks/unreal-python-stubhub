## SmartObjectZoneAnnotations Objects

```python
class SmartObjectZoneAnnotations(ZoneGraphAnnotationComponent)
```
...
- ``affected_lane_tags`` (ZoneGraphTagFilter):  [Read-Write] Filter specifying which lanes the behavior is applied to.
...
- ``behavior_tag`` (ZoneGraphTag):  [Read-Only] Tag to mark the lanes that offers smart objects.
...
- ``smart_object_annotation_data_array`` (Array[SmartObjectAnnotationData]):  [Read-Only] Entry points graph for each ZoneGraphData.
...
<a id="unreal.MassDistanceVisualizationTrait"></a>