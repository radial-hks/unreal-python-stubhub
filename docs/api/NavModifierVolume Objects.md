## NavModifierVolume Objects

```python
class NavModifierVolume(Volume)
```
...
- ``area_class_to_replace`` (type(Class)):  [Read-Write] When setting this value, the modifier volume behavior changes : it will now replace any surface marked by AreaClassToReplace in the volume and replace it with AreaClass.
...
- ``mask_fill_collision_underneath_for_navmesh`` (bool):  [Read-Write] Experimental: if set, the 2D space occupied by the volume box will ignore FillCollisionUnderneathForNavmesh
- ``min_net_update_frequency`` (float):  [Read-Write]
- ``nav_mesh_resolution`` (NavigationDataResolution):  [Read-Write] Experimental: When not set to None, the navmesh tiles touched by the navigation modifier volume will be built
...
(type(Class)):  [Read-Only] When setting this value, the modifier volume behavior changes : it will now replace any surface marked by AreaClassToReplace in the volume and replace it with AreaClass.
...
x.set_area_class_to_replace(new_area_class_to_replace=None) -> None
...
<a id="unreal.NavSystemConfigOverride"></a>