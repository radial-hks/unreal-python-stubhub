## GizmoRectangleComponent Objects

```python
class GizmoRectangleComponent(GizmoBaseComponent)
```
...
- **File**: GizmoRectangleComponent.h
...
- ``direction_x`` (Vector):  [Read-Write]
- ``direction_y`` (Vector):  [Read-Write]
...
- ``hover_size_multiplier`` (float):  [Read-Write]
...
- ``length_x`` (float):  [Read-Write]
- ``length_y`` (float):  [Read-Write]
...
- ``offset_x`` (float):  [Read-Write]
- ``offset_y`` (float):  [Read-Write]
...
- ``orient_y_according_to_camera`` (bool):  [Read-Write] When true, instead of using the provided DirectionY, the component will
...
- ``pixel_hit_distance_threshold`` (float):  [Read-Write]
...
- ``segment_flags`` (uint8):  [Read-Write]
...
<a id="unreal.IntervalGizmoActor"></a>