## NiagaraLensEffectBase Objects

```python
class NiagaraLensEffectBase(NiagaraActor)
```
...
- ``allow_multiple_instances`` (bool):  [Read-Write] Are multiple instances of the effect allowed?
...
- ``base_authored_fov`` (float):  [Read-Write] FOVs that differ from this may cause adjustments in positioning.
...
- ``desired_relative_transform`` (Transform):  [Read-Write] Relative offset from the camera (where X is out from the camera)
  Might be changed slightly when the FOV is different than expected.
- ``emitters_to_treat_as_same`` (Array[type(Class)]):  [Read-Write] If an effect class in this array is currently playing, do not play this effect.
...
- ``niagara_component`` (NiagaraComponent):  [Read-Only] Pointer to System component
...
- ``replicates`` (bool):  [Read-Write] If true, this actor will replicate to remote machines
  see: SetReplicates()
- ``reset_when_retriggered`` (bool):  [Read-Write] When an instance of this effect is retriggered (because more than one instance is not allowed)
...
<a id="unreal.NiagaraRendererProperties"></a>