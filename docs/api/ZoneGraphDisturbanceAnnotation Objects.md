## ZoneGraphDisturbanceAnnotation Objects

```python
class ZoneGraphDisturbanceAnnotation(ZoneGraphAnnotationComponent)
```
...
- **File**: ZoneGraphDisturbanceAnnotation.h
...
- ``affected_lane_tags`` (ZoneGraphTagFilter):  [Read-Write] Filter specifying which lanes the Annotation is applied to.
...
- ``danger_annotation_tag`` (ZoneGraphTag):  [Read-Write] Tag to mark the lanes that should be fled.
...
- ``enable_debug_drawing`` (bool):  [Read-Write]
- ``escape_lane_tags`` (ZoneGraphTagFilter):  [Read-Write] Filter specifying which lanes can be used during Disturbance.
...
- ``ideal_span_length`` (float):  [Read-Write] Ideal span length for lane subdivision. Each lane will have between 2 and 8 spans.
...
- ``obstacle_annotation_tag`` (ZoneGraphTag):  [Read-Write] Tag to mark the lanes influenced by an obstacle.
...
<a id="unreal.ZoneGraphFleeBehavior"></a>