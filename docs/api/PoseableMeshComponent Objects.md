...
def get_bone_transform_by_name(bone_name: Name,
                               bone_space: BoneSpaces) -> Transform
```
...
def copy_pose_from_skeletal_component(
        component_to_copy: SkeletalMeshComponent) -> None
```
...
<a id="unreal.PostProcessComponent"></a>