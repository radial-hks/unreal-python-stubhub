## AesRoofAssetData Objects

```python
class AesRoofAssetData(AesAssetData)
```
...
- ``tag_data`` (AesRoofAssetTagData):  [Read-Write] 资产标签数据

<a id="unreal.AesRoofAssetData.__init__"></a>
...
def __init__(
    ...
    tag_data: AesRoofAssetTagData = [
        RoofShape.FLAT, 0.000000, 1, [255, 255, 255, 255],
        [[""], -1, 1200.000000], [[""], -1, 1200.000000], 30.000000
    ]
) -> None
```

<a id="unreal.AesRoofAssetData.tag_data"></a>

#### tag\_data

```python
@property
def tag_data() -> AesRoofAssetTagData
```

(AesRoofAssetTagData):  [Read-Write] 资产标签数据
...
```python
@tag_data.setter
def tag_data(value: AesRoofAssetTagData) -> None
```

<a id="unreal.AesSlotAssetTagData"></a>