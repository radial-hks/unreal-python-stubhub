## VirtualTextureAdapter Objects

```python
class VirtualTextureAdapter(Texture)
```
...
- ``override_with_texture_format`` (Texture2D):  [Read-Write] Optional UTexture object that has the final texture format that we would like to use.
...
- ``texture`` (Texture):  [Read-Write] The UTexture object to wrap.
- ``tile_border_size`` (int32):  [Read-Write] Page tile border size. (Will be rounded up to multiple of 2).
- ``tile_size`` (int32):  [Read-Write] Page tile size. (Will be rounded up to power of 2).
- ``use_default_tile_sizes`` (bool):  [Read-Write] Set to true if we want to use the default project virtual texture tile settings.
...
<a id="unreal.VirtualTextureBuilder"></a>