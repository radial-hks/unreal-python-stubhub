## MovieGraphBurnInWidget Objects

```python
class MovieGraphBurnInWidget(UserWidget)
```
...
<a id="unreal.MovieGraphBurnInWidget.update_for_graph"></a>

#### update_for_graph

```python
def update_for_graph(graph_pipeline: MovieGraphPipeline,
                     evaluated_config: MovieGraphEvaluatedConfig,
                     camera_index: int, camera_name: str) -> None
```

x.update_for_graph(graph_pipeline, evaluated_config, camera_index, camera_name) -> None
Called on the first temporal and first spatial sample of each output frame.

Args:
    graph_pipeline (MovieGraphPipeline): The graph pipeline the burn-in is for. This will be consistent throughout a burn-in widget's life.
    evaluated_config (MovieGraphEvaluatedConfig): The evaluated graph that was used to generate this output frame.
    camera_index (int32): 
    camera_name (str):

<a id="unreal.MovieGraphCameraSettingNode"></a>