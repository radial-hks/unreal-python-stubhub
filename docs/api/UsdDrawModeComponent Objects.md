## UsdDrawModeComponent Objects

```python
class UsdDrawModeComponent(PrimitiveComponent)
```
...
- ``bounds_color`` (LinearColor):  [Read-Write]
- ``bounds_max`` (Vector):  [Read-Write]
- ``bounds_min`` (Vector):  [Read-Write] Separate vectors instead of an FBox so we can get Sequencer tracks for these as is
...
- ``card_geometry`` (UsdModelCardGeometry):  [Read-Write]
- ``card_texture_x_neg`` (Texture2D):  [Read-Write]
- ``card_texture_x_pos`` (Texture2D):  [Read-Write]
- ``card_texture_y_neg`` (Texture2D):  [Read-Write]
- ``card_texture_y_pos`` (Texture2D):  [Read-Write]
- ``card_texture_z_neg`` (Texture2D):  [Read-Write]
- ``card_texture_z_pos`` (Texture2D):  [Read-Write]
...
- ``draw_mode`` (UsdDrawMode):  [Read-Write]
...
(Vector):  [Read-Write] Separate vectors instead of an FBox so we can get Sequencer tracks for these as is
...
```python
@property
def draw_mode() -> UsdDrawMode
```

(UsdDrawMode):  [Read-Write]
...
```python
@draw_mode.setter
def draw_mode(value: UsdDrawMode) -> None
```
...
```python
@bounds_color.setter
def bounds_color(value: LinearColor) -> None
```
...
```python
@property
def card_geometry() -> UsdModelCardGeometry
```

(UsdModelCardGeometry):  [Read-Write]
...
def card_geometry(value: UsdModelCardGeometry) -> None
```
...
```python
@property
def card_texture_x_pos() -> Texture2D
```
...
def card_texture_x_pos(value: Texture2D) -> None
```
...
```python
@property
def card_texture_y_pos() -> Texture2D
```
...
def card_texture_y_pos(value: Texture2D) -> None
```
...
```python
@property
def card_texture_z_pos() -> Texture2D
```
...
def card_texture_z_pos(value: Texture2D) -> None
```
...
```python
@property
def card_texture_x_neg() -> Texture2D
```
...
def card_texture_x_neg(value: Texture2D) -> None
```
...
```python
@property
def card_texture_y_neg() -> Texture2D
```
...
def card_texture_y_neg(value: Texture2D) -> None
```
...
```python
@property
def card_texture_z_neg() -> Texture2D
```
...
def card_texture_z_neg(value: Texture2D) -> None
```
...
x.set_texture_for_face(face, texture) -> None
Set Texture for Face

Args:
    face (UsdModelCardFace): 
    texture (Texture2D):
...
x.get_texture_for_face(face) -> Texture2D
...
<a id="unreal.UsdProjectSettings"></a>