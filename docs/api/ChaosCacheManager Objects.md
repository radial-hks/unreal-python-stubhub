## ChaosCacheManager Objects

```python
class ChaosCacheManager(Actor)
```
...
- ``cache_collection`` (ChaosCacheCollection):  [Read-Write] The Cache Collection asset to use for this observer. This can be used for playback and record simultaneously
  across multiple components depending on the settings for that component.
- ``cache_mode`` (CacheMode):  [Read-Write] How to use the cache - playback or record
...
- ``observed_components`` (Array[ObservedComponent]):  [Read-Write] List of observed objects and their caches
...
- ``start_mode`` (StartMode):  [Read-Write] How to trigger the cache record or playback, timed will start counting at BeginPlay, Triggered will begin
...
- ``start_time`` (float):  [Read-Write] Defines the (random access) time that represents the rest pose of the components managed by this cache.
...
(float):  [Read-Write] Defines the (random access) time that represents the rest pose of the components managed by this cache.
When in Play mode, the components are set to the state provided by the caches at this evaluated time.
...
x.trigger_component_by_cache(cache_name) -> None
...
Args:
    cache_name (Name): Cache name to trigger
...
x.trigger_component(component) -> None
Triggers a component to play or record.
...
Args:
    component (PrimitiveComponent): Component to trigger
...
x.set_current_time(current_time) -> None
Set Current Time
...
x.reset_single_transform(index) -> None
...
Args:
    index (int32): Index of the component to reset
...
x.reset_all_component_transforms() -> None
...
x.enable_playback_by_cache(cache_name, enable) -> None
...
x.enable_playback(index, enable) -> None
...
<a id="unreal.ChaosCachePlayer"></a>