## HeterogeneousVolumeComponent Objects

```python
class HeterogeneousVolumeComponent(MeshComponent)
```
...
- **Module**: Engine
- **File**: HeterogeneousVolumeComponent.h
...
- ``frame_transform`` (Transform):  [Read-Only]
...
- ``issue_blocking_requests`` (bool):  [Read-Write]
...
- ``lighting_downsample_factor`` (float):  [Read-Write]
...
- ``shadow_bias_factor`` (float):  [Read-Write]
...
- ``shadow_step_factor`` (float):  [Read-Write]
...
- ``step_factor`` (float):  [Read-Write]
- ``streaming_mip_bias`` (float):  [Read-Write] Bias to apply to the calculated mip level to stream at. This property essentially influences the distance from the
...
- ``volume_resolution`` (IntVector):  [Read-Write]
...
```python
@property
def volume_resolution() -> IntVector
```
...
<a id="unreal.HeterogeneousVolumeComponent.frame"></a>
...
x.set_volume_resolution(new_value) -> None
...
x.set_streaming_mip_bias(new_value) -> None
...
x.set_start_frame(new_value) -> None
Set Start Frame
...
x.set_frame_rate(new_value) -> None
Set Frame Rate
...
x.set_end_frame(new_value) -> None
Set End Frame

Args:
    new_value (float):

<a id="unreal.HeterogeneousVolumeComponent.play"></a>
...
<a id="unreal.HeterogeneousVolume"></a>