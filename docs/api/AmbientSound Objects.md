...
- ``audio_component`` (AudioComponent):  [Read-Only] Audio component that handles sound playing
...
<a id="unreal.AmbientSound.play"></a>
...
def fade_in(fade_in_duration: float,
            fade_volume_level: float = 1.000000) -> None
```
...
def adjust_volume(adjust_volume_duration: float,
                  adjust_volume_level: float) -> None
```
...
<a id="unreal.AimOffsetBlendSpace"></a>