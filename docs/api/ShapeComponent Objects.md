...
(Color):  [Read-Write] Color used to draw the shape.
...
```python
@shape_color.setter
def shape_color(value: Color) -> None
```
...
x.set_line_thickness(thickness) -> None
Set the LineThickness

Args:
    thickness (float):

<a id="unreal.BoxComponent"></a>