## RigUnit\_Multiply\_VectorVector Objects

```python
class RigUnit_Multiply_VectorVector(RigUnit_BinaryVectorOp)
```

Rig Unit Multiply Vector Vector

**C++ Source:**

- **Plugin**: ControlRig
- **Module**: ControlRig
- **File**: RigUnit_Vector.h

**Editor Properties:** (see get_editor_property/set_editor_property)

- ``argument0`` (Vector):  [Read-Write]
- ``argument1`` (Vector):  [Read-Write]
- ``result`` (Vector):  [Read-Write]

<a id="unreal.RigUnit_Multiply_VectorVector.__init__"></a>

#### \_\_init\_\_

```python
def __init__(argument0: Vector = [0.000000, 0.000000, 0.000000],
             argument1: Vector = [0.000000, 0.000000, 0.000000],
             result: Vector = [0.000000, 0.000000, 0.000000]) -> None
```

<a id="unreal.RigUnit_Add_VectorVector"></a>