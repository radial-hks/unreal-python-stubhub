## JoinedSVGDynamicMeshComponent Objects

```python
class JoinedSVGDynamicMeshComponent(SVGBaseDynamicMeshComponent)
```
...
- ``coloring`` (JoinedSVGMeshColoring):  [Read-Write]
...
- ``main_color`` (LinearColor):  [Read-Write]
...
- ``shape_parameters_list`` (Set[SVGShapeParameters]):  [Read-Write]
...
<a id="unreal.SVGDynamicMeshesContainerActor"></a>