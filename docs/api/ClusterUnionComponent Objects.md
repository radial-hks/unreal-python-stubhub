## ClusterUnionComponent Objects

```python
class ClusterUnionComponent(PrimitiveComponent)
```
...
- ``clustered_components_references`` (Array[ComponentReference]):  [Read-Write] These are the statically clustered components. These should
...
- ``gravity_group_index_override`` (int32):  [Read-Write] If set to a value not equal to -1, will manually set the cluster union's gravity group
...
- ``on_component_added_event`` (OnClusterUnionAddedComponent):  [Read-Write]
...
- ``on_component_bounds_changed_event`` (OnClusterUnionBoundsChanged):  [Read-Write]
...
- ``on_component_removed_event`` (OnClusterUnionRemovedComponent):  [Read-Write]
...
```python
@property
def on_component_added_event() -> OnClusterUnionAddedComponent
```

(OnClusterUnionAddedComponent):  [Read-Write]
...
def on_component_added_event(value: OnClusterUnionAddedComponent) -> None
```
...
```python
@property
def on_component_removed_event() -> OnClusterUnionRemovedComponent
```

(OnClusterUnionRemovedComponent):  [Read-Write]
...
def on_component_removed_event(value: OnClusterUnionRemovedComponent) -> None
```
...
```python
@property
def on_component_bounds_changed_event() -> OnClusterUnionBoundsChanged
```

(OnClusterUnionBoundsChanged):  [Read-Write]
...
def on_component_bounds_changed_event(
        value: OnClusterUnionBoundsChanged) -> None
```
...
x.set_is_anchored(is_anchored) -> None
Set Is Anchored
deprecated: SetIsAnchored is deprecated, Use SetSimulatePhysics instead
...
x.remove_component_from_cluster(component) -> None
...
def remove_component_bones_from_cluster(component: PrimitiveComponent,
                                        bone_ids: Array[int]) -> None
```

x.remove_component_bones_from_cluster(component, bone_ids) -> None
Remove Component Bones from Cluster
...
x.get_primitive_components() -> Array[PrimitiveComponent]
...
def add_component_to_cluster(component: PrimitiveComponent,
                             bone_ids: Array[int],
                             rebuild_geometry: bool = True) -> None
```

x.add_component_to_cluster(component, bone_ids, rebuild_geometry=True) -> None
...
<a id="unreal.ClusterUnionReplicatedProxyComponent"></a>