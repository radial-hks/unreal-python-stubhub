...
```python
@property
def dynamic_mesh_component() -> DynamicMeshComponent
```
...
```python
@property
def enable_compute_mesh_pool() -> bool
```

(bool):  [Read-Write] Control whether the DynamicMeshPool will be created when requested via GetComputeMeshPool()
...
def enable_compute_mesh_pool(value: bool) -> None
```
...
x.release_compute_mesh(mesh) -> bool
...
x.release_all_compute_meshes() -> None
...
x.get_dynamic_mesh_component() -> DynamicMeshComponent
...
x.get_compute_mesh_pool() -> DynamicMeshPool
...
x.free_all_compute_meshes() -> None
...
x.allocate_compute_mesh() -> DynamicMesh
