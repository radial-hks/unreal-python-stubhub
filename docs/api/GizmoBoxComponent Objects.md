## GizmoBoxComponent Objects

```python
class GizmoBoxComponent(GizmoBaseComponent)
```
...
- ``enable_axis_flip`` (bool):  [Read-Write]
...
- ``line_thickness`` (float):  [Read-Write]
...
- ``remove_hidden_lines`` (bool):  [Read-Write]
...
<a id="unreal.GizmoCircleComponent"></a>