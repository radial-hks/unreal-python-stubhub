...
- ``intensity`` (float):  [Read-Write] The intensity at which the vector field is applied.
...
- ``tightness`` (float):  [Read-Write] How tightly particles follow the vector field.
...
- ``vector_field`` (VectorField):  [Read-Write] The vector field asset.
...
```python
@vector_field.setter
def vector_field(value: VectorField) -> None
```

<a id="unreal.VectorFieldComponent.intensity"></a>
...
<a id="unreal.VectorFieldComponent.tightness"></a>
...
```python
@tightness.setter
def tightness(value: float) -> None
```
...
<a id="unreal.WindDirectionalSourceComponent"></a>