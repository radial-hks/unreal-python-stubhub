## SparseVolumeTextureViewer Objects

```python
class SparseVolumeTextureViewer(Info)
```
...
- ``sparse_volume_texture_viewer_component`` (SparseVolumeTextureViewerComponent):  [Read-Only]
...
```python
@property
def sparse_volume_texture_viewer_component(
) -> SparseVolumeTextureViewerComponent
```

(SparseVolumeTextureViewerComponent):  [Read-Only]

<a id="unreal.ControlPointMeshComponent"></a>