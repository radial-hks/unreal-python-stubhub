## AnimGraphNode\_StateMachine Objects

```python
class AnimGraphNode_StateMachine(AnimGraphNode_StateMachineBase)
```
...
- **Module**: AnimGraph
- **File**: AnimGraphNode_StateMachine.h
...
- ``node`` (AnimNode_StateMachine):  [Read-Write] Runtime state machine representation (empty; populated during compilation)
...
<a id="unreal.AnimGraphNode_Sync"></a>