## FieldSystemComponent Objects

```python
class FieldSystemComponent(PrimitiveComponent)
```
...
- ``field_system`` (FieldSystem):  [Read-Write] Field system asset to be used to store the construction fields. This asset is not required anymore and will be deprecated soon.
...
- ``is_chaos_field`` (bool):  [Read-Write] If enabled the field will be used by all the chaos solvers
- ``is_editor_only`` (bool):  [Read-Write] If true, the component will be excluded from non-editor builds
- ``is_world_field`` (bool):  [Read-Write] If enabled the field will be pushed to the world fields and will be available to materials and niagara
...
- ``supported_solvers`` (Array[ChaosSolverActor]):  [Read-Write] List of chaos solvers that will use the field
...
```python
@property
def field_system() -> FieldSystem
```

(FieldSystem):  [Read-Only] Field system asset to be used to store the construction fields. This asset is not required anymore and will be deprecated soon.
...
def apply_strain_field(enabled: bool, position: Vector, radius: float,
                       magnitude: float, iterations: int) -> None
```
...
def apply_radial_vector_falloff_force(enabled: bool, position: Vector,
                                      radius: float, magnitude: float) -> None
```
...
<a id="unreal.FieldSystemMetaData"></a>