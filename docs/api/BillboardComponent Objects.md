...
- ``is_screen_size_scaled`` (bool):  [Read-Write]
...
- ``opacity_mask_ref_val`` (float):  [Read-Write] The billboard is not rendered where texture opacity < OpacityMaskRefVal
...
<a id="unreal.BillboardComponent.sprite"></a>
...
```python
@sprite.setter
def sprite(value: Texture2D) -> None
```
...
<a id="unreal.BillboardComponent.u"></a>
...
<a id="unreal.BillboardComponent.ul"></a>
...
<a id="unreal.BillboardComponent.v"></a>
...
<a id="unreal.BillboardComponent.vl"></a>
...
```python
@property
def opacity_mask_ref_val() -> float
```

(float):  [Read-Write] The billboard is not rendered where texture opacity < OpacityMaskRefVal
...
def opacity_mask_ref_val(value: float) -> None
```
...
x.set_opacity_mask_ref_val(ref_val) -> None
Changed the opacity masked used by this component

Args:
    ref_val (float):

<a id="unreal.SpriteComponent"></a>