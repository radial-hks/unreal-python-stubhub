## TemplateSequenceActor Objects

```python
class TemplateSequenceActor(Actor)
```
...
- ``binding_override`` (TemplateSequenceBindingOverrideData):  [Read-Write] The override for the template sequence's root object binding. See SetBinding.
...
- ``sequence_player`` (TemplateSequencePlayer):  [Read-Write]
- ``spawn_collision_handling_method`` (SpawnActorCollisionHandlingMethod):  [Read-Write] Controls how to handle spawning this actor in a situation where it's colliding with something else. "Default" means AlwaysSpawn here.
- ``sprite_scale`` (float):  [Read-Write] The scale to apply to any billboard components in editor builds (happens in any WITH_EDITOR build, including non-cooked games).
- ``tags`` (Array[Name]):  [Read-Write] Array of tags that can be used for grouping and categorizing.
- ``template_sequence`` (SoftObjectPath):  [Read-Write]
...
```python
@property
def sequence_player() -> TemplateSequencePlayer
```

(TemplateSequencePlayer):  [Read-Only]
...
```python
@property
def template_sequence() -> SoftObjectPath
```
...
```python
@property
def binding_override() -> TemplateSequenceBindingOverrideData
```
...
<a id="unreal.TemplateSequencePlayer"></a>