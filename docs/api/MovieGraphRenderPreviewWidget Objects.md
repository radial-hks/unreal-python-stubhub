## MovieGraphRenderPreviewWidget Objects

```python
class MovieGraphRenderPreviewWidget(UserWidget)
```
...
<a id="unreal.MovieGraphRenderPreviewWidget.on_initialized_for_pipeline"></a>

#### on_initialized_for_pipeline

```python
def on_initialized_for_pipeline(pipeline: MovieGraphPipeline) -> None
```

x.on_initialized_for_pipeline(pipeline) -> None
On Initialized for Pipeline

Args:
    pipeline (MovieGraphPipeline):

<a id="unreal.MovieGraphVideoOutputNode"></a>