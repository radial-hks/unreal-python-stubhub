## WaterBodyOceanComponent Objects

```python
class WaterBodyOceanComponent(WaterBodyComponent)
```
...
- **Plugin**: Water
- **Module**: Water
- **File**: WaterBodyOceanComponent.h
...
- ``center_on_water_zone`` (bool):  [Read-Write] If enabled, oceans will always center their mesh/bounds on the owning water zone by using a saved location that is updated whenever the ocean mesh is rebuilt.
- ``collision_extents`` (Vector):  [Read-Write]
...
- ``ocean_extents`` (Vector2D):  [Read-Write] The extent of the ocean, centered around water zone to which the ocean belongs.
...
```python
@property
def ocean_extents() -> Vector2D
```

(Vector2D):  [Read-Only] The extent of the ocean, centered around water zone to which the ocean belongs.

<a id="unreal.WaterBodyRiver"></a>