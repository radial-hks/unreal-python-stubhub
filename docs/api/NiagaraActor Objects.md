## NiagaraActor Objects

```python
class NiagaraActor(Actor)
```
...
```python
@property
def niagara_component() -> NiagaraComponent
```
...
def set_destroy_on_system_finish(
        should_destroy_on_system_finish: bool) -> None
```
...
x.get_destroy_on_system_finish() -> bool
Returns true if the system will destroy on finish

Returns:
    bool:

<a id="unreal.NiagaraComponent"></a>