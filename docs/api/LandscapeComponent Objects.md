## LandscapeComponent Objects

```python
class LandscapeComponent(PrimitiveComponent)
```
...
- ``collision_mip_level`` (int32):  [Read-Write] Heightfield mipmap used to generate collision
...
- ``forced_lod`` (int32):  [Read-Write] Forced LOD level to use when rendering
...
- ``layer_allow_list`` (Array[LandscapeLayerInfoObject]):  [Read-Write] List of layers allowed to be painted on this component
...
- ``lighting_lod_bias`` (int32):  [Read-Write] LOD level Bias to use when lighting building via lightmass, -1 Means automatic LOD calculation based on ForcedLOD + LODBias
- ``lightmap_type`` (LightmapType):  [Read-Write]
- ``lod_bias`` (int32):  [Read-Write] LOD level Bias to use when rendering
...
- ``override_hole_material`` (MaterialInterface):  [Read-Write]
- ``override_material`` (MaterialInterface):  [Read-Write]
...
- ``section_base_x`` (int32):  [Read-Only] X offset from global components grid origin (in quads)
- ``section_base_y`` (int32):  [Read-Only] Y offset from global components grid origin (in quads)
...
- ``simple_collision_mip_level`` (int32):  [Read-Write] Heightfield mipmap used to generate simple collision
...
- ``static_lighting_resolution`` (float):  [Read-Write] StaticLightingResolution overriding per component, default value 0 means no overriding
...
```python
@property
def override_material() -> MaterialInterface
```
...
def override_material(value: MaterialInterface) -> None
```
...
```python
@property
def override_hole_material() -> MaterialInterface
```
...
def override_hole_material(value: MaterialInterface) -> None
```
...
x.set_forced_lod(forced_lod) -> None
Set Forced LOD
...
def editor_get_paint_layer_weight_by_name_at_location(
        location: Vector, paint_layer_name: Name) -> float
```
...
def editor_get_paint_layer_weight_at_location(
        location: Vector, paint_layer: LandscapeLayerInfoObject) -> float
```
...
<a id="unreal.LandscapeGizmoActor"></a>