...
- ``ambient_zone_settings`` (InteriorSettings):  [Read-Write] Impacts sounds that have "Apply Ambient Volumes" set to true in their Sound Class, based on whether the sound sources and the player are inside or outside the audio volume
...
- ``enabled`` (bool):  [Read-Write] Whether this volume is currently enabled and able to affect sounds
...
- ``settings`` (ReverbSettings):  [Read-Write] Reverb settings to use for this volume.
...
- ``submix_override_settings`` (Array[AudioVolumeSubmixOverrideSettings]):  [Read-Write] Submix effect chain override settings. Will override the effect chains on the given submixes when the conditions are met.
- ``submix_send_settings`` (Array[AudioVolumeSubmixSendSettings]):  [Read-Write] Submix send settings to use for this volume. Allows audio to dynamically send to submixes based on source and listener locations relative to this volume.
...
<a id="unreal.AudioVolume.priority"></a>
...
<a id="unreal.AudioVolume.enabled"></a>
...
<a id="unreal.AudioVolume.settings"></a>

#### settings

```python
@property
def settings() -> ReverbSettings
```
...
```python
@property
def ambient_zone_settings() -> InteriorSettings
```

(InteriorSettings):  [Read-Only] Impacts sounds that have "Apply Ambient Volumes" set to true in their Sound Class, based on whether the sound sources and the player are inside or outside the audio volume
...
```python
@property
def submix_send_settings() -> Array[AudioVolumeSubmixSendSettings]
```
...
```python
@property
def submix_override_settings() -> Array[AudioVolumeSubmixOverrideSettings]
```
