## NavLinkProxy Objects

```python
class NavLinkProxy(Actor)
```
...
- ``on_smart_link_reached`` (SmartLinkReachedSignature):  [Read-Write]
...
- ``point_links`` (Array[NavigationLink]):  [Read-Write] Navigation links (point to point) added to navigation data
...
- ``smart_link_comp`` (NavLinkCustomComponent):  [Read-Only] Smart link: can affect path following
- ``smart_link_is_relevant`` (bool):  [Read-Write] Smart link: toggle relevancy
...
```python
@property
def point_links() -> Array[NavigationLink]
```
...
```python
@property
def nav_links() -> Array[NavigationLink]
```
...
```python
@property
def on_smart_link_reached() -> SmartLinkReachedSignature
```

(SmartLinkReachedSignature):  [Read-Write]
...
def on_smart_link_reached(value: SmartLinkReachedSignature) -> None
```
...
<a id="unreal.NavLocalGridManager"></a>