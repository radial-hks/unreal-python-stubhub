...
(MaterialInterface):  [Read-Only] Translucent material to blend on top of this mesh. Mesh will be rendered twice - once with a base material and once with overlay material
...
(float):  [Read-Only] The max draw distance for overlay material. A distance of 0 indicates that overlay will be culled using primitive max distance.
...
```python
@property
def enable_material_parameter_caching() -> bool
```
...
def set_color_parameter_value_on_materials(
        parameter_name: Name, parameter_value: LinearColor) -> None
```

x.set_color_parameter_value_on_materials(parameter_name, parameter_value) -> None
Set all occurrences of Vector Material Parameters with ParameterName in the set of materials of the SkeletalMesh to ParameterValue
...
def prestream_textures(seconds: float,
                       prioritize_character_textures: bool,
...
x.prestream_mesh_lo_ds(seconds) -> bool
...
Args:
    seconds (float): Number of seconds to force all LODs to be resident

Returns:
    bool: bool                                                        True if streaming was successfully requested
