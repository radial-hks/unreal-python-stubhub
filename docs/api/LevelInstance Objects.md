## LevelInstance Objects

```python
class LevelInstance(Actor)
```
...
- ``desired_runtime_behavior`` (LevelInstanceRuntimeBehavior):  [Read-Write]
...
- ``level_instance_component`` (LevelInstanceComponent):  [Read-Only]
...
- ``property_overrides`` (LevelInstancePropertyOverrideAsset):  [Read-Only]
...
- ``world_asset`` (World):  [Read-Write] Level LevelInstance
...
<a id="unreal.LevelInstanceComponent"></a>