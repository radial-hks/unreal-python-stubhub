## WaterBodyIsland Objects

```python
class WaterBodyIsland(Actor)
```
...
- ``water_curve_settings`` (WaterCurveSettings):  [Read-Write]
- ``water_heightmap_settings`` (WaterBodyHeightmapSettings):  [Read-Write]
- ``water_weightmap_settings`` (Map[Name, WaterBodyWeightmapSettings]):  [Read-Write]
...
```python
@property
def water_curve_settings() -> WaterCurveSettings
```
...
def water_curve_settings(value: WaterCurveSettings) -> None
```
...
```python
@property
def water_weightmap_settings() -> Map[Name, WaterBodyWeightmapSettings]
```
